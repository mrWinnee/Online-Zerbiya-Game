class Scene3 extends Phaser.Scene {
    ...
    create(){
        ...
        const turnIndicator = this.add.text(config.width/2-48 , config.height - 96, `Red Turn`);
        ...
        socket.on('sideClickedVerified', ({sideName, x, y, turn})=>{
            if(sideName.includes('P1')){ // check if the side is horizontal or vertical
                //theObject.destroy();
                switch (turn) {
                    case 'Red':
        ...
        for (let i = 0; i < hSidePattern.length; i++) {
            for (let j = 0; j < hSidePattern[i].length; j++) {
                if (hSidePattern[i][j] === 1) {
                    const name = 'X' + j + 'Y' + (i-1) + 'P3' + '|' + 'X' + j + 'Y' + i + 'P1';
        ...
        for (let i = 0; i < vSidePattern.length; i++) {
            for (let j = 0; j < vSidePattern[i].length; j++) {
                if (vSidePattern[i][j] === 1) {
                    const name = 'X' + (j-1) + 'Y' + i + 'P4' + '|' + 'X' + j + 'Y' + i + 'P2';
